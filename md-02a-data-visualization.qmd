...
head(gapminder_summary_continent_year)
```
...
# https://www.perplexity.ai/search/how-do-I-njaRLy6PREKyuacmA6nLDg#0
```
...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here: geom_bar is disegned to make bar plots with the "count" on the y-axis
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe: it divides the x-axis starting from 40 until 85 in chunks of size 2.5, basically defining the "bin size"
...
#im not sure what the numbers below the plot are
```
